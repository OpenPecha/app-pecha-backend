...
async def read_terms(language: str | None,
    ...
    await get_all_terms(token=authentication_credential.credentials, language=language)


@terms_router.post("/terms", status_code=status.HTTP_201_CREATED)
async def create_terms(language: str | None, create_term_request: CreateTermRequest,
                       authentication_credential: Annotated[HTTPAuthorizationCredentials, Depends(oauth2_scheme)]):
    await create_new_term(
