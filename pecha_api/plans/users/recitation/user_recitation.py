...
class UserRecitation(Base):
    ...
    id = Column[UUID](UUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id = Column[UUID](UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    recitation_id = Column[UUID](UUID(as_uuid=True), ForeignKey('recitation.id', ondelete='CASCADE'), nullable=False)
