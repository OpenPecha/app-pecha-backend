...
Revision ID: 6d6e2057a835
Revises: e86de3ca779a
Create Date: 2025-08-28 16:58:22.017368
...
revision: str = '6d6e2057a835'
down_revision: Union[str, None] = 'e86de3ca779a'
...
def upgrade() -> None:
    ...
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_plan_status') THEN CREATE TYPE user_plan_status AS ENUM ('not_started', 'active', 'paused', 'completed', 'abandoned'); END IF; END $$;")
    op.execute("DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'content_type') THEN CREATE TYPE content_type AS ENUM ('text', 'audio', 'video', 'image', 'source_reference'); END IF; END $$;")
    ...
    sa.Column('status', postgresql.ENUM('not_started', 'active', 'paused', 'completed', 'abandoned', name='user_plan_status', create_type=False), nullable=True),
    ...
    sa.Column('content_type', postgresql.ENUM('text', 'audio', 'video', 'image', 'source_reference', name='content_type', create_type=False), nullable=False),
