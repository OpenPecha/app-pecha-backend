...
async def test_pipeline_runs_segment_toc_and_returns_new_texts_when_any_new():
    ...
    with patch("pecha_api.text_uploader.pipeline.verify_admin_access", return_value=True), patch(
        ...
        MockMappingService.return_value.trigger_mapping.assert_awaited_once_with(
            text_ids={"T1": "pecha_instance_1"},
...
async def test_pipeline_skips_segment_and_toc_when_no_new_texts():
    ...
    with patch("pecha_api.text_uploader.pipeline.verify_admin_access", return_value=True), patch(
        ...
        called_payload = MockMappingService.return_value.trigger_mapping.await_args.kwargs[
            "text_upload_request"
        ]
        assert called_payload == request
