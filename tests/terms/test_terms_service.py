...
async def test_create_new_term():
    with patch("pecha_api.terms.terms_service.verify_admin_access", return_value=True), \
        ...
        mock_create_term.return_value = AsyncMock(titles={"en": "New Term"}, slug="new-term")
        create_term_request = CreateTermRequest(slug="new-term", titles={"en": "New Term"})
...
async def test_update_existing_term():
    with patch("pecha_api.terms.terms_service.verify_admin_access", return_value=True), \
        ...
        mock_update_term_titles.return_value = AsyncMock(titles={"en": "Updated Term"}, slug="updated-term")
        update_term_request = UpdateTermRequest(titles={"en": "Updated Term"})
