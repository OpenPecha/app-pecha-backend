...
@pytest.mark.asyncio
async def test_update_plan_details_increase_total_days():
    """Test increasing total_days creates new plan items"""
    plan_id = uuid.uuid4()
    author_email = "author@example.com"
    ...
    update_request = UpdatePlanRequest(total_days=8)
    ...
    with patch("pecha_api.plans.cms.cms_plans_service.SessionLocal") as mock_session_local, \
         ...
         patch("pecha_api.plans.cms.cms_plans_service.update_plan") as mock_update_plan, \
         patch("pecha_api.plans.cms.cms_plans_service.get_plan_items_by_plan_id") as mock_get_items, \
         patch("pecha_api.plans.cms.cms_plans_service.save_plan_items") as mock_save_items:
        ...
        updated_items = existing_items + [MagicMock(spec=PlanItem, day_number=i) for i in range(6, 9)]
        mock_get_items.side_effect = [existing_items, updated_items]
        ...
        saved_items = mock_save_items.call_args.kwargs["plan_items"]
        assert len(saved_items) == 3
        assert all(item.plan_id == plan_id for item in saved_items)
        assert all(item.created_by == author_email for item in saved_items)
        assert [item.day_number for item in saved_items] == [6, 7, 8]
        
        assert response.total_days == 8
...
async def test_update_plan_details_decrease_total_days():
    ...
    with patch("pecha_api.plans.cms.cms_plans_service.SessionLocal") as mock_session_local, \
        ...
        remaining_items = existing_items[:5]
        mock_get_items.side_effect = [existing_items, remaining_items]
